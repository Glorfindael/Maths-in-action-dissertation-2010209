...
G.add_edges_from(inter_cluster_edges)  # Add inter-cluster edges

# Assign weights to edges with the revised scheme
for (u, v) in G.edges():
...
def simulate_sirs_abm(G, beta, gamma, delta, epsilon, alpha, initial_infected, steps):
    N_bots = int(0.025 * N)  # 5% of the network as bots
    bot_nodes = np.random.choice(G1.nodes(), size=N_bots, replace=False)# Randomly choosing bot nodes
...
# Create a non-polarized graph (Erdős-Rényi graph) as an example
...
# Parameters for the ABM simulation
beta = 0.2  
gamma = 0.05  
delta = 0.005
epsilon = 0.000
alpha = 0.4
initial_infected = 3/200  # Initial fraction of infected nodes
steps = 500
...
for i in range(1):
    # Simulate SEIRS model using ABM
...
plt.plot(mean_I_p, color='red', linewidth=2)  # Plot the mean line in bold
